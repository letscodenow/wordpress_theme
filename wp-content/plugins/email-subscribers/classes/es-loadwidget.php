...
class es_cls_widget
{
	public static function load_subscription($arr)
	{
		...
		$es = $es . "<div class=\"newsletter-form\">";
		...
		$es = $es . '<div class="es_textbox">';
			$es = $es . '<input class="es_textbox_class shadow-z-1" placeholder="Your Email address" name="es_txt_email_pg" id="es_txt_email_pg" onkeypress="if(event.keyCode==13) es_submit_pages('.$url.')" value="" maxlength="225" type="email">';
		$es = $es . '</div>';
		$es = $es . '<div class="es_button">';
			$es = $es . '<button class="es_textbox_button btn btn-material-red" name="es_txt_button_pg" id="es_txt_button_pg" onClick="return es_submit_pages('.$url.')" type="submit">'.__('Subscribe Now!', ES_TDOMAIN).'</button>';
		$es = $es . '</div>';
		if( $es_name != "YES" )
		{
